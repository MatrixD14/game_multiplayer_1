...
public class criente1 extends Component {
  ...
  private IntBuffer animCache, animBufferCache, DirCache, DirBufferCache, moveCache, moveBufferCache;
  ...
  private animAmigo animat;
  private moveVision player;

  public Texture[] sprites = new Texture[3];
  ...
  void start() {
    ...
    moveCache = BufferUtils.createIntBuffer(maxPlayer);
  ...
  void repeat() {
    ...
    for (int i = 0; i < maxPlayer; i++) {
      if (remotePlay[i] != null && remoteId[i] != 0) {
        ...
        int anim = animCache.get(i);
        int dir = DirBufferCache.get(i);
        boolean move = moveBufferCache.get(i) != 0;
        animat.PlayAnimation(remotePlay[i], sprites[0], dir, i, move, anim);
      } 
    }
    swap();
    ...
    if (txt.startsWith("id:")) {
      ...
      runOnMain(
          () -> {
            ...
            if (player == null) player = localPlayer.findComponent("moveVision");
            new AsyncTask(
                new AsyncRunnable() {
                  public Object onBackground(Object input) {
                    try {
                      ...
                      while (connected && socket != null && !socket.isClosed()) {
                        ...
                        int curret = (player != null) ? player.getCurrentAnim() : 0, animfc = (player != null) ? player.getAnimFC() : 0, move = (player != null && player.getMove()) ? 1 : 0;
                        ...
                        sb.append("anim:").append(myId).append(':').append(curret).append(':').append(animfc).append(':').append(move).append('\n');
      ...
      protocolo.handleAnim(txt, myId, animCache, DirCache, moveCache, remoteId, maxPlayer);
    } else {
  ...
  private void swap() {
    ...
    IntBuffer tmpanim = animCache, tmpDir = DirCache, tmpmove = moveCache;
    ...
    DirCache = DirBufferCache;
    DirBufferCache = tmpDir;

    moveCache = moveBufferCache;
    moveBufferCache = tmpmove;
  }
}
